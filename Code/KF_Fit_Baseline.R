# The Kalman Filter Fit function modified from the KFTemplate available under MiscFuncs pakcage
# This function was used for evaluating the baseline model

KFfit <- function(  param,              # model parameters
                    data,               # a matrix containing the data ie Y
                    cov_info, m,        # other parameters       
  ...
  for(t in 1:T){
    ...
    new <- KFadvance(obs=data[t,],oldmean=Xpost,oldvar=Vpost,A=A,B=B,C=C,D=D,E=E,F=F,W=W,V=V,marglik=TRUE,log=TRUE,na.rm=na.rm)
