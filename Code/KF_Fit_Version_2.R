KFfit_updated <- function(  param,              # model parameters
  ...
  for(t in 1:T){
    ...
    P <- matrix(cov_info$momentum[t] * gamma, 1, 1)
    
    # this bit calls KF advance
    new <- kfadvance_udpated(obs=data[t,],oldmean=Xpost,oldvar=Vpost,A=A,B=B,C=C,D=D,E=E,F=F,P=P,W=W,V=V,marglik=TRUE,log=TRUE,na.rm=na.rm)
